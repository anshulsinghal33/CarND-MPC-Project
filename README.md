...
The number of points(N) and the time interval(dt) define the prediction horizon (T). The number of points impacts the controller performance as well. The values chosen for N and dt are 10 and 0.1, respectively. I chose these values after trying out different values for N in the range of 10 to 25, larger values of N made the model slow and also sometimes act erratically. While choosing the value for 'dt' I went with 0.1 after trying different values like 0.05, 0.15, 0.25, etc, as adjusting these values by even a small value made the system to behave erratically. Making the N higher and dt smaller such as N = 25 and dt = 0.01, was making the car behave erratic and wobble around the middle of the lane as the actuations were made too often specially on a straight line.

## Polynomial Fitting and MPC Preprocessing
The waypoints provided by the simulator are transformed to the car coordinate system at ./src/main.cpp from line 104 to line 111. Then a 3rd-degree polynomial is fitted to the transformed waypoints. These polynomial coefficients are used to calculate the cte and epsi later on. They are used by the solver as well to create a reference trajectory.

## Model Predictive Control with Latency
To acount for the latency the model equation were altered, as the original equations depended upon the actuations from the previous timestep but with a delay of 100ms (which happens to be the timestep interval) and the actuations are applied another timestep later.(See MPC.cpp lines 151-154).
